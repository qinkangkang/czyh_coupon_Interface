package com.innee.czyhInterface.web.coupon;
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (Constant.cLogFilterList.contains(hm.getMethod().getName())) {
			...
			if (map.containsKey("ticket") && StringUtils.isNotBlank(map.get("ticket").toString())) {
				...
				Element element = ticketToIdCache.get(map.get("ticket").toString());
				if (element != null) {
					customerLogBean.setCustomerId(element.getObjectValue().toString());
				}
			}
			...
			customerLogBean.setClientIp(request.getRemoteAddr());
