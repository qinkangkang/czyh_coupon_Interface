...
public class FxlService {
	...
	public String getCustomerIdByTicket(String ticket, Integer clientType) {
		...
		try {
			ticketElement = redisService.getValue(ticket, "TicketToId");
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (ticketElement == null) {
			...
			if (tCustomerTicket != null) {
				redisService.putCache("TicketToId", ticket, tCustomerTicket.getFcustomerId());
	...
	public CustomerDTO getCustomerByTicket(String ticket, Integer clientType) {
		...
		try {
			CustomerEentity = redisService.getValue(customerId, "CustomerEentity");
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (CustomerEentity == null) {
			...
			if (customer == null) {
				...
				redisService.putCache("CustomerEentity", customerId, mapper.toJson(customerDTO));
			}
		} else {
