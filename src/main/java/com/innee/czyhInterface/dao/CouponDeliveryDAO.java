...
public interface CouponDeliveryDAO
	...
	@Query("from TCouponDelivery t where t.id = ?1 and (t.fcustomerId = ?2 or t.fcustomerId is null) and t.fdeliverTime >= ?3")
	TCouponDelivery getCouponbycustomer(String couponDeliveryId, String customerId,Date date);
	
	@Query("from TCouponDelivery t where (t.fcustomerId = ?1 or t.fcustomerId is null ) and t.TCouponInformation.id = ?2")
	TCouponDelivery getCouponbyCustomer(String customerId, String couponId);
	
	@Modifying
	@Query("update TCouponDelivery t set t.fcustomerId = ?1 where t.fcustomerId = ?2")
