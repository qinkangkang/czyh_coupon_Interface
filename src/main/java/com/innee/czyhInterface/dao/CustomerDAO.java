...
public interface CustomerDAO extends JpaRepository<TCustomer, String>, JpaSpecificationExecutor<TCustomer> {
	...
	/*
	 * @Query(
	 * "select count(t.id) from TCustomer t where t.id != (select s.TCustomer.id from TSponsor s where s.id = ?1) and t.fusername = ?2 and t.fstatus < 999"
	...
	@Query("select t from TCustomer t where t.fweixinId = ?1 and t.ftype = ?2 and t.id != ?3 and t.fstatus < 999")
	TCustomer getByOpenidAndTypeAndNotCustomerId(String openid, Integer ftype, String customerId);

	@Query("select t from TCustomer t where t.id = ?1 and t.fstatus < 999")
	TCustomer findCustomerIdByName(String customerId);
}